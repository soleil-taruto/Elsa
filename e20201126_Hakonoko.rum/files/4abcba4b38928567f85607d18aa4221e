...
{
	public static class プレイヤー位置矯正処理
	{
				...
				case 2: // 右下のみ
					if (-16 < A2.RelativePoint.X && -16 < A2.RelativePoint.Y)
					{
						if (Game.I.Player.YSpeed < 0.0 || yTurned) // ? 上昇中 || 天井に当たった。
						{
							...
							if (A2.RelativePoint.X < (強制上昇 ? -8 : A2.RelativePoint.Y))
							{
						...
						else // ? 接地中 || 落下中
						{
							...
							bool 強制落下 = 1 <= DDInput.DIR_2.GetInput();
							...
							{
								// 落下最高速度 == 7.0 --> 8ピクセル以上入り込んでくることは無いはず。--> -16 + 8 == -8

								if (A2.RelativePoint.Y < -8)
								{
									A2.RelativePoint.Y = -16;
									Touch = Touch_e.GROUND;
								}
