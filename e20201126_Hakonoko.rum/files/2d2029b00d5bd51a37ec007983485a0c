		...
		public override IEnumerable<bool> E_Draw()
		{
			...
			for (; ; )
			{
				...
				// 跳ね返り
				{
					...
					if (Game.I.Map.GetCell(Common.ToTablePoint(this.X - R, this.Y)).IsEnemyWall())
					{
						this.XAddSign = 1;
						bounced = true;
					}
					else if (Game.I.Map.GetCell(Common.ToTablePoint(this.X + R, this.Y)).IsEnemyWall())
					{
						this.XAddSign = -1;
						bounced = true;
					}

					if (Game.I.Map.GetCell(Common.ToTablePoint(this.X, this.Y - R)).IsEnemyWall())
					{
						this.YAddSign = 1;
						bounced = true;
					}
					else if (Game.I.Map.GetCell(Common.ToTablePoint(this.X, this.Y + R)).IsEnemyWall())
					{
						this.YAddSign = -1;
						bounced = true;
					}
					...
					{
						if (Game.I.Map.GetCell(Common.ToTablePoint(this.X - R, this.Y - R)).IsEnemyWall()) // ? 左上に衝突
						{
							this.XAddSign = 1;
							this.YAddSign = 1;
							bounced = true;
						}
						else if (Game.I.Map.GetCell(Common.ToTablePoint(this.X + R, this.Y - R)).IsEnemyWall()) // ? 右上に衝突
						{
							this.XAddSign = -1;
							this.YAddSign = 1;
							bounced = true;
						}
						else if (Game.I.Map.GetCell(Common.ToTablePoint(this.X - R, this.Y + R)).IsEnemyWall()) // ? 左下に衝突
						{
							this.XAddSign = 1;
							this.YAddSign = -1;
							bounced = true;
						}
						else if (Game.I.Map.GetCell(Common.ToTablePoint(this.X + R, this.Y + R)).IsEnemyWall()) // ? 右下に衝突
						{
							this.XAddSign = -1;
							this.YAddSign = -1;
							bounced = true;
					...
					if (bounced)
					{
						double mid_x = (int)(this.X / Consts.TILE_W) * Consts.TILE_W + Consts.TILE_W / 2;
						double mid_y = (int)(this.Y / Consts.TILE_H) * Consts.TILE_H + Consts.TILE_H / 2;
