	...
	public static class DDSaveData
	{
		public static void Save()
		{
			...
			// Donut3 用のセーブデータ
			{
				...
				lines.Add("" + (DDGround.UseVirtualSubScreenSize ? 1 : 0));
			...
			// アプリ固有のセーブデータ
			{
				...
				foreach (string savedData in Ground.I.SaveDataSlots)
				{
					string line;

					if (savedData == null)
						line = Consts.SERIALIZED_NULL;
					else
						line = Consts.SERIALIZED_NOT_NULL_PREFIX + savedData;
		...
		public static void Load()
		{
			...
			try // Donut3 のセーブデータ
			{
				...
				DDGround.UseVirtualSubScreenSize = int.Parse(lines[c++]) != 0;
			...
			Load_Delay = () =>
			{
				...
				try // アプリ固有のセーブデータ
				{
					...
					Ground.I.MessageSpeed = SCommon.ToInt(lines[c++],
						GameConsts.MESSAGE_SPEED_MIN,
						GameConsts.MESSAGE_SPEED_MAX,
						GameConsts.MESSAGE_SPEED_DEF
						);
					...
					for (int index = 0; index < GameConsts.SAVE_DATA_SLOT_NUM; index++)
					{
						...
						if (line == Consts.SERIALIZED_NULL)
							...
							savedData = line.Substring(Consts.SERIALIZED_NOT_NULL_PREFIX.Length);

						Ground.I.SaveDataSlots[index] = savedData;
					}
