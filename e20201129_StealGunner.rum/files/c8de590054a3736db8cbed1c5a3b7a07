...
namespace Charlotte.Games
{
	public class TitleMenu : IDisposable
	{
		...
		public void Perform()
		{
			...
			for (; ; )
			{
				selectIndex = this.SimpleMenu.Perform("横シュー・テストコード / タイトルメニュー(仮)", items, selectIndex);
					...
					case 0:
						{
							...
							using (new Game())
							{
								Game.I.Script = new Script_ステージ0001(); // 仮？
								Game.I.Perform();
							}
		...
		private class 背景Task : DDTask
		{
			...
			private class SnowPanelTask : DDTask
			{
				...
				public override IEnumerable<bool> E_Task()
				{
					for (; ; )
					{
						...
						if (DDUtils.Random.Real2() < this.PutRate)
						{
							double r = DDUtils.AToBRate(this.RMin, this.RMax, DDUtils.Random.Real());

							this.Snows.Add(new SnowInfo()
							{
								X = DDUtils.Random.Real2() * DDConsts.Screen_W,
								Y = -10.0,
								R = r,
								XAdd = DDUtils.Random.Real2() * -3.0,
								YAdd = r * 1.5,
							}
							.Task
							);
				...
				private class SnowInfo : DDTask
				{
					...
					public override IEnumerable<bool> E_Task()
					{
						for (; ; )
						{
							...
							DDDraw.DrawZoom(this.R / (DDGround.GeneralResource.WhiteCircle.Get_W() / 2.0));
							DDDraw.DrawEnd();
							DDDraw.Reset();
			...
			{
#if false // fill version
				public override IEnumerable<bool> E_Task()
				{
					for (int slide = 0; ; slide += 3, slide %= Ground.I.Picture.Wall0001.Get_W())
					{
						for (int x = -slide; x < DDConsts.Screen_W; x += Ground.I.Picture.Wall0001.Get_W())
							for (int y = 0; y < DDConsts.Screen_H; y += Ground.I.Picture.Wall0001.Get_H())
								DDDraw.DrawSimple(Ground.I.Picture.Wall0001, x, y);
