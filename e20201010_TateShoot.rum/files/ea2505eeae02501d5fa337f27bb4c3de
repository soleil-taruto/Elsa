...
namespace Charlotte.Games.Enemies
{
	public static class EnemyCommon
	{
		...
		private static void LittleFairyDraw(FairyInfo fairy, int color)
		{
			...
			Game.I.AddEnemyCrash(DDCrashUtils.Circle(new D2Point(fairy.Enemy.X, fairy.Enemy.Y), 13.0), fairy.Enemy);
		}
		...
		private static void BigFairyDraw(FairyInfo fairy, int color)
		{
			...
			Game.I.AddEnemyCrash(DDCrashUtils.Circle(new D2Point(fairy.Enemy.X, fairy.Enemy.Y), 30.0), fairy.Enemy);
		}
		...
		{
			if (DDUtils.IsOut(new D2Point(enemy.X, enemy.Y), new D4Rect(0, 0, Consts.FIELD_W, Consts.FIELD_H))) // ? フィールド外
				return;
				...
				case 101: // 自機狙い_Normal_R
					{
						...
						if (mod == 0 && SCommon.IsRange(div, 3, 12))
						{
							...
							Game.I.Enemies.Add(Enemy_Tama_01.Create(enemy.X, enemy.Y, TAMA_KIND_e.NORMAL, color, 4.0, 0.0));
						}
					}
					break;
				...
				case 200: // 三角形_Normal_W
					{
						...
						if (mod == 0 && SCommon.IsRange(div, 0, 5))
						{
							...
							for (int c = 0; c <= div; c++)
							{
								...
								Game.I.Enemies.Add(Enemy_Tama_01.Create(enemy.X, enemy.Y, TAMA_KIND_e.NORMAL, color, 5.0, angle));
							}
						}
					}
					break;
		...
		private static void DropItem(Enemy enemy, DROP_ITEM_TYPE_e dropItemType, int num = 1, double rMin = 0.0, double rBnd = 0.0)
		{
			if (num == 1)
			{
				Game.I.Enemies.Add(Enemy_Item.Create(enemy.X, enemy.Y, dropItemType));
			}
			else
			{
				...
				for (int index = 0; index < num; index++)
				{
					...
					addingItems[index] = Enemy_Item.Create(
