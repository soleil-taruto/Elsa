		...
		public override IEnumerable<bool> E_Draw()
		{
			// 薄い壁にくっついて撃つと、壁の向こうに射出されてしまうのを防ぐ
			{
				foreach (int xd in new int[] { 0, 20, 40 })
				{
					...
					{
						if (Game.I.Map.GetCell(Common.ToTablePoint(new D2Point(this.X - xd * (this.FacingLeft ? -1 : 1), this.Y + yd))).Tile.IsWall())
						{
			...
			for (int frame = 0; ; frame++)
			{
				...
				// 跳ね返り
				{
					...
					if (Game.I.Map.GetCell(Common.ToTablePoint(this.X, this.Y + R)).Tile.IsWall() && 0.0 < yAdd)
					{
						...
						while (
							!Game.I.Map.GetCell(Common.ToTablePoint(this.X, this.Y)).Tile.IsWall() &&
							Game.I.Map.GetCell(Common.ToTablePoint(this.X, this.Y + R)).Tile.IsWall()
							)
							this.Y--;
					}
