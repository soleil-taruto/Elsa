...
namespace Charlotte.Games
{
	public class Game : IDisposable
	{
		...
		public void Perform()
		{
			...
			this.Reborn_X = this.Player.X;
			this.Reborn_Y = this.Player.Y;
			...
			for (this.Frame = 0; ; this.Frame++)
			{
				...
				if (1 <= this.Player.DeadFrame) // プレイヤー死亡中の処理
				{
					...
					if (frame == 0) // init
					{
						DDGround.EL.Add(new 波紋効果(this.Player.X, this.Player.Y).Task);
					}
				}
			endDead:
			...
			endReborn:
				...
				// プレイヤー位置矯正
				{
					プレイヤー位置矯正処理.Perform(ref this.Player.X, ref this.Player.Y, v => v.IsWall());

					if (プレイヤー位置矯正処理.Touch == プレイヤー位置矯正処理.Touch_e.GROUND)
					{
					...
					else
					{
						...
						if (プレイヤー位置矯正処理.Touch == プレイヤー位置矯正処理.Touch_e.ROOF)
						{
							this.Player.JumpFrame = 0;
				...
				const double PL_CRASH_W = Consts.TILE_W - 1;
				const double PL_CRASH_H = Consts.TILE_H - 1;
				...
				this.Map.Design.DrawWall(
					DDGround.Camera.X,
					DDGround.Camera.Y,
					DDGround.Camera.X / (this.Map.W * Consts.TILE_W - DDConsts.Screen_W),
					DDGround.Camera.Y / (this.Map.H * Consts.TILE_H - DDConsts.Screen_H)
					);
				this.DrawTiles();
				this.Player.Draw();
				...
				// ゴール
				{
					...
					if (
						this.Map.GetCell(Common.ToTablePoint(new D2Point(this.Player.X - R, this.Player.Y - R))).IsGoal() ||
						this.Map.GetCell(Common.ToTablePoint(new D2Point(this.Player.X + R, this.Player.Y - R))).IsGoal() ||
						this.Map.GetCell(Common.ToTablePoint(new D2Point(this.Player.X - R, this.Player.Y + R))).IsGoal() ||
						this.Map.GetCell(Common.ToTablePoint(new D2Point(this.Player.X + R, this.Player.Y + R))).IsGoal()
						)
		...
		{
			this.Player.X = this.Reborn_X;
			this.Player.Y = this.Reborn_Y;
		...
		private void ゴールした()
		{
			DDEngine.EachFrame(); // ゴールした瞬間はこのフレームなので MainScreen を描画する必要がある。
			DDMain.KeepMainScreen(); // ...そうしてから KeepMainScreen する。
