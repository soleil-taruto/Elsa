		...
		public void Perform()
		{
			...
			for (this.Frame = 0; ; this.Frame++)
			{
				...
				// プレイヤー入力
				{
					...
					if (1 <= this.Player.ShootingFrame) // カウンタ・インクリメント
					{
						this.Player.ShootingFrame++;

						if (Consts.PLAYER_SHOOTING_FRAME_MAX < this.Player.ShootingFrame)
							this.Player.ShootingFrame = 0;
					}
				...
				if (1 <= this.Player.DamageFrame) // プレイヤー・ダメージ中の処理
				{
					...
					DDPrint.SetPrint(
						(int)this.Player.X - DDGround.ICamera.X,
						(int)this.Player.Y - DDGround.ICamera.Y - 50
						);
					DDPrint.SetBorder(new I3Color(0, 0, 255));
					DDPrint.Print("[ヒットバック中]");
					DDPrint.Reset();
				}
			endDamage:
			...
			endInvincible:
					...
					)
				{
					...
					{
						double SPEED = Consts.PLAYER_SPEED;

						SPEED = Math.Min(this.Player.MoveFrame, SPEED); // 走り出し時に加速する。
				...
				// プレイヤー位置矯正
				{
					...
					for (int c = 0; c < 20; c++) // ハマり防止
					{
						bool xL2 = this.Map.GetCell(Common.ToTablePoint(this.Player.X - Consts.PLAYER_側面判定Pt_X * 2, this.Player.Y - Consts.PLAYER_側面判定Pt_Y)).Tile.IsWall();
						bool xL1 = this.Map.GetCell(Common.ToTablePoint(this.Player.X - Consts.PLAYER_側面判定Pt_X * 1, this.Player.Y - Consts.PLAYER_側面判定Pt_Y)).Tile.IsWall();
						bool xR1 = this.Map.GetCell(Common.ToTablePoint(this.Player.X + Consts.PLAYER_側面判定Pt_X * 1, this.Player.Y - Consts.PLAYER_側面判定Pt_Y)).Tile.IsWall();
						bool xR2 = this.Map.GetCell(Common.ToTablePoint(this.Player.X + Consts.PLAYER_側面判定Pt_X * 2, this.Player.Y - Consts.PLAYER_側面判定Pt_Y)).Tile.IsWall();
