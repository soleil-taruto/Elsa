...
namespace Charlotte.Games
{
	...
	public class Player
	{
		...
		public int Level
		{
			get
			{
				return this.Power / Consts.PLAYER_POWER_PER_LEVEL;
			}
		}
		...
		public void Shot()
		{
			...
			if (1 <= this.BornFrame)
			{
				...
				if (DDUtils.IsOut(new D2Point(pl_x, pl_y), new D4Rect(0, 0, Consts.FIELD_W, Consts.FIELD_H))) // 画面外からは撃てない。
				{
					return;
				}
			}
			...
			if (1 <= this.SlowFrame) // 低速移動
			{
				if (Game.I.Frame % 4 == 0)
				{
					...
					Game.I.Shots.Add(Shot_Laser.Create(pl_x, pl_y - 10.0, this.Level));
				}
			}
			else // 高速移動
			{
					...
					case 1:
						{
							if (Game.I.Frame % 3 == 0)
							{
								Game.I.Shots.Add(Shot_Normal.Create(
									pl_x,
									pl_y - 5.0,
									0.0
									));
					...
					case 3:
						{
							if (Game.I.Frame % 3 == 0)
							{
								Game.I.Shots.Add(Shot_Strong.Create(
									pl_x,
									pl_y - 5.0,
									0.0
									));
					...
					case 5:
						{
							if (Game.I.Frame % 4 == 0)
							{
								for (int c = -1; c <= 1; c += 2)
								{
									Game.I.Shots.Add(Shot_Wave.Create(
										pl_x + 20.0 * c,
										pl_y - 5.0,
										0.3 * c,
		...
		public void Bomb()
		{
			Game.I.Shots.Add(Shot_Bomb.Create(this.X, this.Y));
		}
