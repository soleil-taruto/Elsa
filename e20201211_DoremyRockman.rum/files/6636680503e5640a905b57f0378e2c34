	...
	public static class マップ移動
	{
		...
		public static void Perform(Map currMap, Map nextMap, D2Point currMapCamera, int exitDirection, GameStatus.StartPlayerStatusInfo plStatus)
		{
...

			{
				...
				double xd = (x2 - x1) * Consts.TILE_W;
				double yd = (y2 - y1) * Consts.TILE_H;
				...
				double l = 0.0;
				double t = 0.0;
				double r = nextMap.W * Consts.TILE_W - DDConsts.Screen_W;
				double b = nextMap.H * Consts.TILE_H - DDConsts.Screen_H;
				...
				double pl_r = nextMap.W * Consts.TILE_W - PL_XY_MARGIN;
				double pl_b = nextMap.H * Consts.TILE_H - PL_XY_MARGIN;
		...
		private static void DrawToScreen(DDSubScreen screen, Map map, D2Point mapCamera)
		{
			...
			int l = cam_l / Consts.TILE_W;
			int t = cam_t / Consts.TILE_H;
			int r = cam_r / Consts.TILE_W;
			int b = cam_b / Consts.TILE_H;
			...
			using (screen.Section())
			{
				...
				for (int x = l; x <= r; x++)
				{
					for (int y = t; y <= b; y++)
					{
						double cell_x = x * Consts.TILE_W + Consts.TILE_W / 2;
						double cell_y = y * Consts.TILE_H + Consts.TILE_H / 2;

						map.GetCell(x, y).Tile.Draw(cell_x - cam_l, cell_y - cam_t);
					}
				}
			}
		}
