...
namespace Charlotte.GameCommons
{
	public static class DDDraw
	{
		...
		private static void DrawPic_Main(DDPicture picture, ILayoutInfo layout)
		{
			...
			{
				rvx = (double)DDGround.RealScreen_W / DDConsts.Screen_W;
				rvy = (double)DDGround.RealScreen_H / DDConsts.Screen_H;
			}
			else
			{
				rvx = 1.0;
...

			{
				...
				if (u != null)
				{
					...
					if (
						Extra.IntPos ?
						DX.DrawModiGraph(
							SCommon.ToInt(u.LTX * rvx),
							SCommon.ToInt(u.LTY * rvy),
							SCommon.ToInt(u.RTX * rvx),
							SCommon.ToInt(u.RTY * rvy),
							SCommon.ToInt(u.RBX * rvx),
							SCommon.ToInt(u.RBY * rvy),
							SCommon.ToInt(u.LBX * rvx),
							SCommon.ToInt(u.LBY * rvy),
							picture.GetHandle(),
							1
							)
							!= 0
...

			{
				...
				if (u != null)
				{
					...
					if (
						Extra.IntPos ?
						DX.DrawExtendGraph(
							SCommon.ToInt(u.L * rvx),
							SCommon.ToInt(u.T * rvy),
							SCommon.ToInt(u.R * rvx),
							SCommon.ToInt(u.B * rvy),
							picture.GetHandle(),
							1
							)
							!= 0
				...
				if (u != null)
				{
					if (DDGround.UseVirtualSubScreenSize)
					{
						...
						if (
							Extra.IntPos ?
							DX.DrawExtendGraph(
								SCommon.ToInt(u.X * rvx),
								SCommon.ToInt(u.Y * rvy),
								SCommon.ToInt((u.X + picture.Get_W()) * rvx),
								SCommon.ToInt((u.Y + picture.Get_H()) * rvy),
								picture.GetHandle(),
								1
								)
								!= 0
							...
							DX.DrawExtendGraphF(
								(float)(u.X * rvx),
								(float)(u.Y * rvy),
								(float)((u.X + picture.Get_W()) * rvx),
								(float)((u.Y + picture.Get_H()) * rvy),
								picture.GetHandle(),
								1
								)
								!= 0
		...
		endDraw:
...

			{
				...
				if (u != null)
				{
					...
					LastCrash = DDCrashUtils.Rect(new D4Rect(
						l,
						t,
						r - l,
						b - t
		...
		private static DDCrash LastCrash = DDCrashUtils.None();

		public static bool IsCrashed(D2Point pt)
		{
			return DDCrashUtils.IsCrashed(DDCrashUtils.Point(pt), LastCrash);
		}

		public static bool IsMouseCrashed()
		{
			return IsCrashed(new D2Point(DDMouse.X, DDMouse.Y));
		}
