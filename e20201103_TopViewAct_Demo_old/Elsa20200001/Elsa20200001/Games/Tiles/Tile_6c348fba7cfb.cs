	...
	public abstract class Tile_水辺系 : Tile
	{
		...
		private class DrawTask : DDTask
		{
			...
			private bool Draw_KBKFA(int komaBack, int komaFront, double komaFrontAlpha)
			{
				...
				this.MapTablePoint = new I2Point((int)this.MapCellCenterPoint.X / Consts.TILE_W, (int)this.MapCellCenterPoint.Y / Consts.TILE_H);

				bool friend_4 = this.F_IsFriend(Game.I.Map.GetCell(this.MapTablePoint.X - 1, this.MapTablePoint.Y).Tile);
				bool friend_6 = this.F_IsFriend(Game.I.Map.GetCell(this.MapTablePoint.X + 1, this.MapTablePoint.Y).Tile);
				bool friend_8 = this.F_IsFriend(Game.I.Map.GetCell(this.MapTablePoint.X, this.MapTablePoint.Y - 1).Tile);
				bool friend_2 = this.F_IsFriend(Game.I.Map.GetCell(this.MapTablePoint.X, this.MapTablePoint.Y + 1).Tile);

				bool friend_1 = this.F_IsFriend(Game.I.Map.GetCell(this.MapTablePoint.X - 1, this.MapTablePoint.Y + 1).Tile);
				bool friend_3 = this.F_IsFriend(Game.I.Map.GetCell(this.MapTablePoint.X + 1, this.MapTablePoint.Y + 1).Tile);
				bool friend_7 = this.F_IsFriend(Game.I.Map.GetCell(this.MapTablePoint.X - 1, this.MapTablePoint.Y - 1).Tile);
				bool friend_9 = this.F_IsFriend(Game.I.Map.GetCell(this.MapTablePoint.X + 1, this.MapTablePoint.Y - 1).Tile);
				...
				})
				{
					DDDraw.DrawCenter(
						...
						this.Point.X + info.Dir.X * Consts.MINI_TILE_W / 2,
						this.Point.Y + info.Dir.Y * Consts.MINI_TILE_H / 2
						);
