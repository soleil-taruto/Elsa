...
namespace Charlotte.Games
{
	...
	public class Player
	{
		...
		public void Draw()
		{
			...
			DDDraw.DrawCenter(picture, this.X - DDGround.ICamera.X, this.Y - DDGround.ICamera.Y);
			DDDraw.Reset();
		}
			...
			switch (this.選択武器)
			{
				...
				case 武器_e.BOUNCE:
					if (this.AttackFrame % 25 == 1)
					{
						for (int c = -1; c <= 1; c++)
						{
							Game.I.Shots.Add(new Shot_Bounce(this.X, this.Y, Common.Rotate(this.FaceDirection, c)));
						}
					}
					break;
