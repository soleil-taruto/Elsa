...
using Charlotte.Games.Enemies.神奈子s;
...
namespace Charlotte.Games.Enemies
{
	...
	public static class EnemyCatalog
	{
		...
		private static EnemyInfo[] Tiles = new EnemyInfo[]
		{
			...
			new EnemyInfo("セーブ地点", () => new Enemy_セーブ地点(X, Y)),
			new EnemyInfo("アイテム・武器-Bounce", () => new Enemy_Item(X, Y, Enemy_Item.効用_e.WEAPON_BOUNCE)),
			new EnemyInfo("アイテム・武器-Spread", () => new Enemy_Item(X, Y, Enemy_Item.効用_e.WEAPON_SPREAD)),
			new EnemyInfo("アイテム・武器-Wave", () => new Enemy_Item(X, Y, Enemy_Item.効用_e.WEAPON_WAVE)),
			new EnemyInfo("敵01", () => new Enemy_B0001(X, Y)),
			new EnemyInfo("敵02", () => new Enemy_B0002(X, Y)),
			new EnemyInfo("神奈子", () => new Enemy_神奈子(X, Y)),
			new EnemyInfo("イベント0001", () => new Enemy_イベント0001(X, Y)),
			new EnemyInfo("ハック0001", () => new Enemy_ハック0001(X, Y)),
