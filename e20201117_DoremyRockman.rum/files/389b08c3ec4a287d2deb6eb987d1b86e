...
namespace Charlotte.Games.Attacks
{
	public static class AttackCommon
	{
		...
		{
			bool touchSide_L =
				(側面モード & 4) != 0 && Game.I.Map.GetCell(Common.ToTablePoint(Game.I.Player.X - Consts.PLAYER_側面判定Pt_X, Game.I.Player.Y - Consts.PLAYER_側面判定Pt_Y)).Tile.IsWall() ||
				(側面モード & 2) != 0 && Game.I.Map.GetCell(Common.ToTablePoint(Game.I.Player.X - Consts.PLAYER_側面判定Pt_X, Game.I.Player.Y)).Tile.IsWall() ||
				(側面モード & 1) != 0 && Game.I.Map.GetCell(Common.ToTablePoint(Game.I.Player.X - Consts.PLAYER_側面判定Pt_X, Game.I.Player.Y + Consts.PLAYER_側面判定Pt_Y)).Tile.IsWall();
		...
		public static bool ProcPlayer_脳天()
		{
			...
			if (ret)
			{
				...
				{
					double plY = ((int)((Game.I.Player.Y - Consts.PLAYER_脳天判定Pt_Y) / Consts.TILE_H) + 1) * Consts.TILE_H + Consts.PLAYER_脳天判定Pt_Y;
