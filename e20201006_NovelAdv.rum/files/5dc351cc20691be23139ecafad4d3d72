...
namespace Charlotte.Games
{
	public class TitleMenu : IDisposable
	{
		...
		restart:
			...
			DDEngine.FreezeInput(Game.LONG_INPUT_SLEEP);
			...
			foreach (DDScene scene in DDSceneUtils.Create(900))
			{
				...
				this.DrawWall();
				tipClickScreen.Draw(DDConsts.Screen_W - 120, DDConsts.Screen_H - 30, Math.Min(scene.Numer - 300, (scene.Denom - scene.Numer) - 30));
				DDEngine.EachFrame();
			}
		...
		private IEnumerable<bool> T_DrawWall2()
		{
			...
			for (; ; )
			{
				...
				{
					DDUtils.Approach(ref h, 5.0, 0.95);
