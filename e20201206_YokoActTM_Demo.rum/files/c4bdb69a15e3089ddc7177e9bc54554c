		...
		public void Perform()
		{
			...
			endInvincible:
				...
				// プレイヤー位置矯正
				{
					...
					const double 側面判定Pt_Y = Consts.TILE_H / 2.0;
					...
					const double 脳天判定Pt_Y = Consts.TILE_H;

					bool touchCeiling_L =
						this.Map.GetCell(Common.ToTablePoint(this.Player.X - 脳天判定Pt_X, this.Player.Y - 脳天判定Pt_Y)).Tile.IsWall();
					...
					if (touchCeiling_L && touchCeiling_R)
					{
						...
						{
							double plY = ((int)((this.Player.Y - 脳天判定Pt_Y) / Consts.TILE_H) + 1) * Consts.TILE_H + 脳天判定Pt_Y;
					...
					const double 接地判定Pt_Y = Consts.TILE_H;

					bool touchGround =
						this.Map.GetCell(Common.ToTablePoint(this.Player.X - 接地判定Pt_X, this.Player.Y + 接地判定Pt_Y)).Tile.IsWall() ||
						this.Map.GetCell(Common.ToTablePoint(this.Player.X + 接地判定Pt_X, this.Player.Y + 接地判定Pt_Y)).Tile.IsWall();
					...
					if (touchGround)
					{
						...
						{
							double plY = (int)((this.Player.Y + 接地判定Pt_Y) / Consts.TILE_H) * Consts.TILE_H - 接地判定Pt_Y;
				...
				DDPrint.SetPrint();
				DDPrint.Print(string.Join(" ", DDEngine.FrameProcessingMillis_Worst, this.Player.HP));
