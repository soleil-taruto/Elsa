...
namespace Charlotte.Games
{
	public class Game : IDisposable
	{
		...
		private void DrawAll当たり判定()
		{
			...
			foreach (DDCrash crash in this.GrazeCrashes.Iterate())
				Draw当たり判定(crash, new I3Color(0, 128, 0));

			foreach (DDCrash crash in this.PlayerCrashes.Iterate())
				Draw当たり判定(crash, new I3Color(255, 0, 0));

			foreach (DDCrash crash in this.EnemyCrashes.Iterate())
				Draw当たり判定(crash, new I3Color(255, 255, 255));

			foreach (DDCrash crash in this.ShotCrashes.Iterate())
				Draw当たり判定(crash, new I3Color(0, 255, 255));
		}

		private static void Draw当たり判定(DDCrash crash, I3Color color)
		{
			DDDraw.SetAlpha(0.2);
			...
			switch (crash.Kind)
			{
				case DDCrashUtils.Kind_e.POINT:
					DDDraw.DrawBegin(DDGround.GeneralResource.WhiteCircle, crash.Pt.X, crash.Pt.Y);
					DDDraw.DrawZoom(2.0 / (DDGround.GeneralResource.WhiteCircle.Get_W() / 2.0));
					DDDraw.DrawEnd();
					break;

				case DDCrashUtils.Kind_e.CIRCLE:
					DDDraw.DrawBegin(DDGround.GeneralResource.WhiteCircle, crash.Pt.X, crash.Pt.Y);
					DDDraw.DrawZoom(crash.R / (DDGround.GeneralResource.WhiteCircle.Get_W() / 2.0));
					DDDraw.DrawEnd();
					break;

				// HACK: 新しい判定の形状についてここへ追加..
