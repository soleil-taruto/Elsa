...

					{
						...
						if (1 <= extendedAttack && extendedAttack < 事前入力時間)
						{
							switch (this.Player.Chara)
							{
								case Player.Chara_e.HOMURA:
									{
										if (this.Player.TouchGround && !PlayerTracer.IsStarted())
		...
		private void Pause()
		{
			...
			for (; ; )
			{
				selectIndex = simpleMenu.Perform(
					...
					new string[]
					{
						"キャラクタ切り替え [ 現在のキャラクタ：" + this.Player.Chara + " ]",
						"動作周期強制遅延 [ 現在の設定：" + DDEngine.SlowdownLevel + " ]",
				...
				{
					case 0:
						this.Player.Chara = (Player.Chara_e)(((int)this.Player.Chara + 1) % Enum.GetNames(typeof(Player.Chara_e)).Length);
						break;
